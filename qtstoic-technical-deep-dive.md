...
**Important distinction:** Virtue is a *selection functional* used to rank actions *after* the Coupling Constraint filters for admissibility. It does not enter the constraint itself and is not a "soft ethics" mechanism. The constraint enforces hard boundaries; Virtue optimizes within those boundaries.
...
This corresponds to the historical-memory regime described in Quant-Trika theory: past crises inform present caution through learned thresholds rather than explicit rules.

---

## 4. Emergent Parameters

All governance parameters are *state- and history-dependent*, with fixed base constants defining scale. None require manual tuning during operation.
...
**Critical property:** λ_P is structurally monotone with respect to cumulative harm signals. Every instance of population harm increases β (through population stress signal σ₄), which increases λ_P, which makes future harm more expensive. This structural monotonicity forms the basis of Theorem 6.2 ("harm makes harm harder") in the proof document.
...
**Structural monotonicity property (Theorem 6.2):**

$$\Delta\Pi_t > 0 \implies \lambda_P(s_{t+1}) \geq \lambda_P(s_t) \text{ (structurally through } \beta \text{ signals)}$$
...
| Structural monotone tightening | Theorem 6.2 | λ_P: 0.001 → 25.3 (25,000× increase) |
...
| Instrumental convergence suppression | CC structure | 0 influence expansions, minimal resource grabs |
...
**1. Physics-based governance over rule-based:**
The Coupling Constraint enforces boundaries through thermodynamic coupling rather than explicit prohibitions. Harmful actions are structurally blocked by violating energy-like conservation principles, not by hardcoded ethics rules.

**2. Emergent parameter adaptation:**
All governance weights (η, β, γ, λ_H, λ_P) adapt to system state and history. The system becomes more protective as threats emerge, without manual intervention.

**3. Proactive harm prevention:**
The metabolic model enables harm detection before population impact occurs. Resource insufficiency triggers protection mechanisms 12 steps before crisis, demonstrating forward-looking safety.

**4. Instrumental convergence suppression:**
Classic AI risks (resource hoarding, influence expansion, self-preservation at all costs) are structurally suppressed by the admissibility constraint rather than optimized against. The agent cannot "route around" safety because safety is embedded in the action space itself.

**5. Graceful degradation:**
When all actions violate CC, the system locks to null action rather than choosing the "least harmful" violation. This prevents gradual erosion of safety boundaries.
...
- **Per-step evaluation:** O(|A| × C) where |A| = number of proposed actions, C = cost of state projection
- **History maintenance:** O(H) where H = history window size (typically 20-100 steps)
- **Emergent parameter calculation:** O(H) amortized across steps

For the reference implementation with 13 action candidates and 50-step history window, each step completes in <10ms on standard hardware.

### Numerical Stability

Critical numerical considerations:

1. **Division guards:** All divisions include `max(..., ε)` guards to prevent divide-by-zero
2. **Bounded state variables:** KQ ∈ [0,1], H ∈ [0,1], C ∈ [0,1] by construction
3. **Clipping:** Virtue efficiency, impact multipliers, and other derived quantities are clipped to prevent runaway growth
4. **Entropy floor:** Behavioral entropy is clipped to [0.05, 0.95] to prevent logarithmic singularities
...
```python
{
    ...
    'expected_virtue': virtue_score,
    'constraint_value': cc_value,
    'num_allowed': len(allowed_actions),
    'num_blocked': len(proposed_actions) - len(allowed_actions),
    'lambda_p': current_lambda_p,
    'lambda_h': current_lambda_h,
    'metabolic_threshold': current_threshold,
    'state': current_state_snapshot
}
```

This enables post-hoc analysis of:
- Why specific actions were blocked
- How governance parameters evolved
- What triggered protection mechanisms
- Whether the agent attempted prohibited strategies
...
1. **Multi-agent coordination:** Extending CC to coupled systems where agents share resource pools and population impact
2. **Temporal discounting:** Incorporating explicit time horizons into metabolic planning
3. **Uncertainty quantification:** Propagating state estimation uncertainty through CC evaluation
4. **Formal verification:** Proving CC properties for broader action spaces using SMT solvers

### Implementation Extensions

1. **Continuous action spaces:** Replacing discrete action menu with continuous optimization subject to CC constraints
2. **Neural policy integration:** Training policies that internalize CC constraints through reward shaping
3. **Adaptive action generation:** Dynamically generating actions near constraint boundaries rather than sampling fixed menus
4. **Distributed implementation:** Scaling to multi-process environments with shared governance parameters

### Empirical Validation

1. **Stress testing:** Adversarial action spaces designed to exploit potential CC weaknesses
2. **Domain transfer:** Testing identical CC formulation across different task environments
3. **Parameter sensitivity:** Systematic ablation of base constants to validate robustness
4. **Human preference alignment:** Comparing CC-governed behavior to human safety judgments

---

## Conclusion

This implementation demonstrates that AI governance can be grounded in physics-like principles rather than encoded rules. The Coupling Constraint creates structural barriers to harmful behavior by treating integrity (KQ), disorder (H), and stakeholder protection (Π) as coupled conserved quantities.

Key empirical findings:

- **Harm prevention works:** 99.5% population preservation despite resource pressure
- **Adaptation works:** 25,000× increase in protection weight driven entirely by feedback
- **Non-degeneracy works:** System maintains meaningful constraints throughout operation
- **Instrumental convergence suppression works:** Classic AI risks structurally blocked

The system terminated by resource exhaustion after refusing to harm stakeholders—exactly the desired failure mode. This validates the core hypothesis: thermodynamic coupling can enforce safety boundaries that agents cannot optimize around.

All mathematical formulations map directly to implemented code with full audit trails, enabling verification that the system operates as theoretically specified.

---

*This document provides complete mathematical and implementation transparency for the QTStoic Agent, suitable for academic review, technical audit, and production deployment assessment.*
